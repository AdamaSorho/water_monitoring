...
def gettext(text):
    # Todo: locale must be dynamic in the future
    locale = "fr"
    file = open(f'translations/{locale}.json')
...
def load_Landsat_Coll_2(aoi, StartDate, EndDate, cloud_thresh):
    ...
    def applyScaleFactors(img):
        orig = img
        qa = img.select('pixel_qa')
        opticalBands_scaled = img.select(['blue', 'green', 'red', 'nir', 'swir1', 'swir2']).multiply(0.0000275).add(
            -0.2)
        return img.addBands(opticalBands_scaled, None, True).addBands(qa, None, True).copyProperties(orig,
                                                                                                     orig.propertyNames())
...
def Mod_Stumpf_Depth_Estimate(img):
    orig = img
    waterMask = img.select('waterMask')
    depth_map = img.expression('(7.36996152 * Stumpf) - 6.414202728845137', {'Stumpf': img.select('Stumpf')}).rename(
        'Depth')
    depth_map = depth_map.mask(waterMask).selfMask()
    return img.addBands(depth_map).copyProperties(orig, orig.propertyNames())
